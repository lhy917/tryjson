...
#ifndef LEPT_PARSR_STACK_INIT_SIZE
...
static void *lept_context_push(lept_context *c, size_t size) {
    ...
    if (c->top + size >= c->size) {
        if (c->size == 0)
            c->size = LEPT_PARSR_STACK_INIT_SIZE;
...
static int lept_parse_array(lept_context *c, lept_value *v) {
    ...
    for (;;) {
        ...
        if (*c->json == ',') {
            ...
            memcpy(v->val.arr.e = (lept_value*)malloc(size), lept_context_pop(c, size), size);
            return LEPT_PARSE_OK;
        }
        else {
...
static int lept_parse_object(lept_context *c, lept_value *v) {
    ...
    for(;;) {
        ...
        if (*c->json == ',') {
            ...
            v->type = LEPT_OBJECT;
            v->val.obj.size = size;
            memcpy(v->val.obj.m = (lept_member *)malloc(s), lept_context_pop(c, s), s);
            return LEPT_PARSE_OK;
        }
        else {
